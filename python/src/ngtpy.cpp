...
public:
  ...
  void batchSearch(
    ...
    bool addZero = false;
    auto distanceType = NGT::Index::getObjectSpace().getDistanceType();
    auto indexDimension = NGT::Index::getObjectSpace().getDimension();
    if (distanceType == NGT::Property::DistanceType::DistanceTypeInnerProduct) {
      if (dimension + 1 == indexDimension) {
	addZero = true;
      } else if (dimension != indexDimension) {
	std::stringstream msg;
	msg << "ngtpy::batchSearch: invalid dimension. " << dimension << ":" << indexDimension;
	NGTThrowException(msg);
      }
    }
#endif
...
#ifdef NGT_INNER_PRODUCT
      if (addZero) {
        ngtquery = NGT::Index::allocateObject(qptr, dimension + 1);
        static_cast<float*>(ngtquery->getPointer())[dimension] = 0.0;
      } else {
        ngtquery = NGT::Index::allocateObject(qptr, dimension);
      }
#else
      ngtquery = NGT::Index::allocateObject(qptr, dimension);
#endif

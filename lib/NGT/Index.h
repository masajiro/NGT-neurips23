...
namespace NGT {
  ...
  public:
    ...
    virtual void createIndex(size_t threadNumber = 0, size_t sizeOfRepository = 0) {
      StdOstreamRedirector redirector(redirect);
      redirector.begin();
      try {
...
#ifdef NGT_INNER_PRODUCT
      if (getObjectSpace().getDistanceType() == NGT::ObjectSpace::DistanceType::DistanceTypeInnerProduct) {
        if (queryContainer.getDimension() + 1 == getObjectSpace().getDimension()) {
          queryContainer.pushBack(0.0);
        }
      }
#endif

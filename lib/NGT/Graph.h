...
#define NGT_GRAPH_COMPACT_READ_ONLY_GRAPH
...
namespace NGT {
  ...
  public:
	...
	for (ObjectDistances::iterator ri = results.begin(); ri != results.end(); ri++) {
	  assert(id != (*ri).id);
	  deleteExcessEdges((*ri).id, id, (*ri).distance);
	}
	return;
      }
	...
	return false;
      ...
      void deleteExcessEdges(ObjectID target, ObjectID addID, Distance addDistance, bool identityCheck = true) {
	GraphNode &node = *getNode(target);
	size_t kEdge = property.edgeSizeForCreation;
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
	if (node.size() > kEdge && node.at(kEdge - 1, repository.allocator).distance > addDistance) {
	  for (size_t idx = kEdge; idx < node.size(); idx++) {
	  GraphNode &linkedNode = *getNode(node.at(idx, repository.allocator).id);
	  if ((linkedNode.size() <= kEdge) || node.at(idx, repository.allocator).distance <
	      linkedNode.at(kEdge - 1, repository.allocator).distance) {
	    continue;
	  }
	  ObjectDistance linkedNodeEdge(target, node.at(kEdge, repository.allocator).distance);
#else
	if (node.size() > kEdge && node[kEdge - 1].distance > addDistance) {
	  for (size_t idx = kEdge; idx < node.size(); idx++) {
	    GraphNode &linkedNode = *getNode(node[idx].id);
	    if ((linkedNode.size() <= kEdge) || node[idx].distance < linkedNode[kEdge - 1].distance) {
	      continue;
	    }
	    ObjectDistance linkedNodeEdge(target, node[idx].distance);
#endif
	    try {
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
	      removeEdge(node, node.at(idx, repository.allocator));
#else
	      removeEdge(node, node[idx]);
#endif
	    } catch (Exception &exp) {
	      std::stringstream msg;
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
	      msg << "addEdge: Cannot remove. (a) " << target << "," << addID << "," << node.at(idx, repository.allocator).id << "," << node.at(kEdge, repository.allocator).distance;
#else
	      msg << "addEdge: Cannot remove. (a) " << target << "," << addID << "," << node[idx].id << "," << node[kEdge].distance;
#endif
	      msg << ":" << exp.what();
	    ...
	    try {
	      ...
	      std::stringstream msg;
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
	      msg << "addEdge: Cannot remove. (b) " << target << "," << addID << "," << node.at(idx, repository.allocator).id << "," << node.at(kEdge, repository.allocator).distance;
#else
	      msg << "addEdge: Cannot remove. (b) " << target << "," << addID << "," << node[idx].id << "," << node[kEdge].distance;
#endif
	      msg << ":" << exp.what();
	      NGTThrowException(msg.str());
	    }

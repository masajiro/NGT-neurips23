...
#define NGT_APPENDING_BYNARY
#ifdef NGT_APPENDING_BYNARY
...
#include	"NGT/StorageBasedGraphConstructor.h"
  ...
  NGT::Command::append(Args &args)
  {
	...
	while (getline(is, line)) {
	  if (is.eof()) break;
	  if (dataSize > 0 && lineNo > dataSize) break;
	  vector<float> object;
	  vector<string> tokens;
	  ...
	  for (auto &v : tokens) object.push_back(NGT::Common::strtod(v));
          ...
          if (lineNo % 2000000 == 0) {
            timer.stop();
            std::cerr << "processed " << static_cast<float>(lineNo) / 1000000.0 << "M objects."
	...
	static_cast<GraphIndex&>(index.getIndex()).property.maxMagnitude = maxMag;
      }
	...
	lineNo++;
        ...
        if (lineNo % 1000000 == 0) {
          timer.stop();
	  std::cerr << "appended " << static_cast<float>(lineNo) / 1000000.0 << "M objects.";
	  if (lineNo != id) {
  ...
  void
  NGT::Command::constructGraph(Args &args) {
    const string usage = "Usage: ngt construct-graph [-X 1] [-i index-type(T|g|t|s)] [-O outdgree] [-I indegree] "
      "[-n result-size(k)] [-e epsilon] [-E edge-size] [-m open-mode(r|w)] [-o output-mode] index(input)";
    args.parse("v");
    ...
    try {
      ...
      cerr << "ngt: Error: index is not specified" << endl;
      cerr << usage << endl;
      return;
    }
    ...
    try {
      baseIndexPath = args.get("#2");
    } catch (...) {}

    size_t nOfThreads = args.getl("T", 0);
    size_t indegree = args.getl("I", 10);
    size_t outdegree = args.getl("O", 10);
    
    size_t nOfSearchedObjects = args.getl("N", std::max(indegree, outdegree));
    size_t nOfFinalEdges = args.getl("P", 0);
    char graphType = args.getChar("G", 'o');
    size_t searchSize = args.getl("S", 100);
    auto epsilon = args.getf("E", 0.0);
    bool verbose = args.getBool("v");

    try {
      NGT::StorageBasedGraphConstructor::construct(indexPath, baseIndexPath, graphType, 
						   nOfSearchedObjects, outdegree, indegree, 
						   nOfFinalEdges, searchSize, epsilon, 
						   nOfThreads, verbose);
    } catch (NGT::Exception &err) {
      cerr << "ngt: Error. " << err.what() << endl;
      cerr << usage << endl;
  ...
  NGT::Command::reconstructGraph(Args &args)
  {
    ...
    graphOptimizer.shortductReductionWithLessMemory = srmode == 's' ? true : false;
    ...
    graphOptimizer.shortcutReductionRange = args.getf("R", 68.0);
#else
